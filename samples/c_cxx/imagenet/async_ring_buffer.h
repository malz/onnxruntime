...
class AsyncRingBuffer {
 ...
 public:
  ...
  void ProcessRemain() {
    ...
    size_t len = p_->GetOutputSizeInBytes(count);
    OrtValue* input_tensor = nullptr;
    ORT_THROW_ON_ERROR(OrtCreateTensorWithDataAsOrtValue(allocator_info, output_data, len, input_shape.data(),
                                                         input_shape.size(), ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT,
                                                         &input_tensor));
  ...
  int StartDownloadTasks() {
    ...
    for (DownloadTask* p : tasks_to_launch) {
      if (!threadpool_.CreateAndSubmitThreadpoolWork(ThreadPoolEntry, p)) {
        return -1;
      }
    }
